<?php
class Bannermain extends Controlador
{
    public function index()
    {
	     $this->se_requiere_logueo(true,'Bannermain|index');
       require URL_VISTA.'banners/main.php';
    }

    public function obtener_banners()
    {
	     $this->se_requiere_logueo(true,'Bannermain|index');
       $modelo = $this->loadModel('Bannermain');
       print $modelo->obtener_banners($_POST);
    }

    public function agregar_banner()
    {
	     $this->se_requiere_logueo(true,'Bannermain|agregar_banner');
       require URL_VISTA.'modales/banners/a_principal.php';
    }

    public function editbanner($id)
    {
	     $this->se_requiere_logueo(true,'Bannermain|editbanner');
       $model = $this->loadModel('Bannermain');
       $elm = $model->data_element($id);
       require URL_VISTA.'modales/banners/e_principal.php';
    }
    ...
    public function upload_dropzone($id){
        $this->se_requiere_logueo(true,'Bannermain|agregar_banner');
        $upload_dir = '../public/content/banners/principal/';
        ...
        if(((strpos($_FILES['file']['type'], 'image') !== false) ||
          ...
          if(move_uploaded_file($pic['tmp_name'], $destino_final)){
            ...
            $set_image = $this->loadModel('Bannermain');
